...
[演示站](https://easygpa-demo.guiguisocute.com)复用真实前端页面和虚构班级数据，所有操作只保存在当前浏览器，不连接后端，也不发送邮件。首次进入默认普通模式并停在登录页，点演示浮层中的账号即可登录；顶部常驻的演示入口可随时切换到共治模式（测试版）。普通模式下还能切换演示身份，共治模式下可推进演示时钟。
