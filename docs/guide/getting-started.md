...
[演示站](https://easygpa-demo.guiguisocute.com)复用真实前端页面和虚构班级数据，所有操作只保存在当前浏览器，不连接后端，也不发送邮件。首次进入先以初始化班管的身份选择普通模式或共治模式；顶部常驻的演示入口可随时切换模式，普通模式下还能切换演示身份，共治模式下可推进演示时钟。
